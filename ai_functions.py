...
def get_best_move(fields):
    ...
    for tab in fields:
        for one in tab:
            if one.isQueen:
                moves = get_queen_moves(fields, one.possesed[0], one.possesed[1])
            else:
                print(len(get_moves(fields, one.possesed[0], one.possesed[1]))
                #moves = get_moves(fields, one.possesed[0], one.possesed[1])
                ...
                for move in moves:
                    if one.position[0] == move[1] and one.position[1] == move[0] and has_moved_in_this_turn == 0:
                        if only_attack == 1:
                            ...
                            if check_for_attacks(fields, one.possesed[1], one.possesed[0]):
                                required = 1
                            all_attacks = all_atacks(fields)
                            fields[move[2]][move[3]].possesed = 0
                            fields[move[0]][move[1]].possesed = turn+1
                    ...
                    if is_game_over(fields) == ai_move:
                        ...
                        next_results = get_best_move(fields)
                        actual_best = (-2000,(0,0,0,0))
                        for result in next_results:
                            if result[0] > actual_best:
                                actual_best = result
